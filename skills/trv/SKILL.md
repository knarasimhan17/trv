...
1. From the repository root, run `trv --agent` in the foreground and wait until it exits. If the current branch is ahead of mainline, this reviews the whole stack (every commit plus uncommitted work) against that mainline, like a pull request. If you have no tty, trv opens a review tab in this window without stealing focus; the user will switch to it when ready. Do not background the command. The user may take many minutes; set the command timeout to at least 3600000 ms (1 hour).
