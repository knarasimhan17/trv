...
1. From the repository root, run `trv --agent` in the foreground and wait until it exits. It takes over this terminal for the review, then returns here. Do not background the command and do not open another window. The user may take many minutes; set the command timeout to at least 3600000 ms (1 hour).
